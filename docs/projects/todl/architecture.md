...
**Audience:** engineers new to the codebase. This document is a map — what the
system does, how the pieces fit, and where to look. It describes the code as it
stands today (`@pragmatic-tech-ai/todl` v0.36.x). For the language itself —
types, meta-models, taxonomies, operators, and the model compiler — see
[The TODL language](todl-language.md). TODL is built on
[todl-runtime](../todl-runtime/).
...
- **House style.** OOP — behaviour on classes, no free functions or module-level
  state; Allman braces; PascalCase interfaces and public methods; enums over
  string-literal unions; reused string literals hoisted to `private static readonly`
  constants; new view models extend `Observable`, not `MuralBase`. Generated files
...
*This document is generated from the in-repo `docs/ARCHITECTURE.md`. Keep the two
in sync as the architecture evolves.*
